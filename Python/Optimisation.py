...
def Main(T_tot, readout_time, slew_time, PGW, tau, prob, method):
    ...
    return tprob, time_allocation


# In[96]:


def Pem(r, lim_mag, lim_time, D_mu, D_sig, Loftau, N_ref, L_min, L_max):
    ...
    if not L_min:
        L_min = 4.9370e31
    elif isinstance(L_min, numbers.Number) == False:
        sys.exit('L_min has to either be an empty array or a number.')
    
    if not L_max:
        L_max = 4.9370e33
    elif isinstance(L_max, numbers.Number) == False:
        sys.exit('L_max has to either be an empty array or a number.')
    
    if isinstance(r, numbers.Number) == False:
        sys.exit('Please enter the raidus of the aperture, in meters.')
        
    if isinstance(lim_mag, numbers.Number) == False:
        sys.exit('Please enter the limiting magnitude of the observing telescope.')   
        
    if isinstance(lim_time, numbers.Number) == False:
        sys.exit('please enter the limiting time of the observing telescope that corresponds to the limiting magnitude.')    
    
    if not D_mu:
        D_mu = 200.0e6
    elif isinstance(D_mu, numbers.Number) == False: 
        sys.exit('D_mu has to either be an empty array or a number.')
        
    if not D_sig:
        D_sig = 60.0e6
    elif isinstance(D_sig, numbers.Number) == False: 
        sys.exit('D_sig has to either be an empty array or a number.')    
           
    if not Loftau:
        Loftau = 61
    elif isinstance(Loftau, numbers.Number) == False: 
        sys.exit('Loftau has to either be an empty array or an integer.') 
        
    if not N_ref:
        N_ref = 9.7847e9
    elif isinstance(N_ref, numbers.Number) == False: 
        sys.exit('N_ref has to either be an empty array or an integer.')     
    ...
    scaling_flux = lim_time * scaling_num * np.pi * r **2 
    ...
    for i in range(Loftau):
        ...
        for dist in range(len(R)):
            for mag in range(sample_length):
                ...
                int_value = (1 - scipy.stats.poisson.cdf(scaling_flux, N_exp[dist][mag] * tau[i] * np.pi * r ** 2)) * p_R[dist] * pMdM[mag] * Rstepsize
                prob[i] += int_value
    ...
    plot.xscale('log')
    plot.yscale('log')
    plot.xlabel('Log of observing time $\tau$',fontsize = 20)
    plot.ylabel('Log of detection prob. given the target is at the observing field',fontsize = 20)
